...
public class ECDSAProvider {
	 ...
	 * @param signature wrapped signature to validate
	...
	public boolean verifySignature(byte[] toBeSignedDataBytes, byte[] signingCertificateBytes,
		...
		return ecdsaSigner.verifySignature(inputHash, signature.r, signature.s);
	}
	
	/**
	 * Decode 1609.2-2016 EccP256CurvePoint (6.3.23) into Bouncy Castle ECPublicKeyParameters.
	 * @param publicKey EccP256CurvePoint to decode
	...
	/**
	 * Encode BouncyCastlePublicKeyParementers into 1609.2-2016 EccP256CurvePoint (6.3.23)
	 * @param publicKey ECPublicKeyParameters to encode
	...
	public EccP256CurvePoint encodePublicKey(ECPublicKeyParameters publicKey) throws CryptoException {
		...
		if (publicKey != null) {	
			...
			OctetString compressedValue = new OctetString(EcdsaP256SignatureWrapper.encodeBigInteger(xValue, ECDSAPublicKeyEncodedLength-1));
	 ...
	 * @param issuerCert  issuer certificate used to create the reconstruction value
	 * @param certificate subordinate certificate for which the key is being reconstructed
	...
	public ECPublicKeyParameters reconstructImplicitPublicKey(
												CertificateWrapper issuerCert, CertificateWrapper certificate)
		...
		// Get PU
		EccP256CurvePoint reconstructionKeyValue = certificate.getCertificate().getToBeSigned().getVerifyKeyIndicator().getReconstructionValue();
	 ...
	 * @param issuerCert  issuer certificate used to create the reconstruction value
	 * @param certificate subordinate certificate for which the key is being reconstructed
	 * @param reconstructionKeyValueBytes  bytes of the reconstruction value
	 * @param seedPrivateKeyBytes  bytes of the seed private key
