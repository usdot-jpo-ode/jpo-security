...
/**
 * Helper class that implements encoding and decoding of the EciesP256EncryptedKey (6.3.5)
 */
public class ECIESProvider {
	...
	public ECIESProvider(CryptoProvider cryptoProvider) {
		...
		iesEngine = new IESEngine(new ECDHCBasicAgreement(), new KDF2BytesGenerator(new SHA256Digest()), new Ieee1609dot2Mac1(new SHA256Digest(), 16));
	}

	/**
	 * ECIES encrypt symmetric encryption key bytes
	 * @param aesSymmetricEncryptionKeyBytes symmetric encryption key bytes to encrypt
	 * @param ephemeralPrivateKey ephemeral private key
	 * @param recipientEncryptionPublicKey recipient's asymmetric public key
	 ...
	 * @param aesSymmetricEncryptionKey symmetric encryption key (AES 128 CCM, 16 bytes)
	 * @param recipientEncryptionPublicKey recipient's asymmetric public key
	 ...
	 * @throws CryptoException if encoding of the ephemeral public key fails
	 */
	public EciesP256EncryptedKey encodeEciesP256EncryptedKey(KeyParameter aesSymmetricEncryptionKey, ECPublicKeyParameters recipientEncryptionPublicKey)
		...
		byte[] encryptedKeyAndTag = encrypt(aesSymmetricEncryptionKey.getKey(), ephemeralPrivateKey, recipientEncryptionPublicKey);
	 ...
	 * @param eciesP256EncryptedKey encoded EciesP256EncryptedKey containing the encrypted key
	 * @param recipientEnryptionPrivateKey recipient's asymmetric private key or null to skip decryption
	 * @return decrypted symmetric encryption key or null if recipientEncryptionPrivateKey is null
	 * @throws InvalidCipherTextException if decoding of the symmetric encryption key fails
	 */
	public KeyParameter decodeEciesP256EncryptedKey(EciesP256EncryptedKey eciesP256EncryptedKey, ECPrivateKeyParameters recipientEnryptionPrivateKey)
		...
		byte[] encryptedSymmetricKeyAndTag = ByteArrayHelper.concat(encryptedSymmectricKey.byteArrayValue(), encryptedTag.byteArrayValue());
		
		byte[] aesSymmetricEncryptionKeyBytes = decrypt(encryptedSymmetricKeyAndTag, ephemeralPublicKey, recipientEnryptionPrivateKey);
