...
public class AESProvider {
    ...
    *            if encryption fails
    */
   public byte[] encrypt(KeyParameter key, byte[] nonce, byte[] clearText, int start, int length)
         throws CryptoException {
      ...
      if (nonce.length != nonceLength)
         throw new CryptoException(
               String.format("Invalid parameter nonce value. Expected: %d. Actual: %d", nonceLength, nonce.length));
    ...
    *            if encryption fails
    */
   public byte[] decrypt(KeyParameter key, byte[] nonce, byte[] cipherText, int start, int length)
         throws CryptoException {
      ...
      try {
         ...
         if (bytesProcessed != plainText.length)
            throw new CryptoException(String.format(
               "Couldn't decrypt cipher text. Reason: Bytes processed mismatch: processed %d vs. plain text %d.",
