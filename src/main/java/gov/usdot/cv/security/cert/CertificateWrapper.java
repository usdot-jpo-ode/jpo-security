...
public class CertificateWrapper {
   ...
   protected SecureECPrivateKey encryptionPrivateKey;
   protected ECPublicKeyParameters signingPublicKey;
   protected SecureECPrivateKey signingPrivateKey;
    ...
    *           encoded bytes of the certificate to wrap
    * @param privateKeyReconstructionValueBytes
    *           private key reconstruction value bytes used in private key
    *           reconstruction
   ...
   protected CertificateWrapper(CryptoProvider cryptoProvider, byte[] certificateBytes,
         byte[] privateKeyReconstructionValueBytes, SecureECPrivateKey seedPrivateKey)
         throws CertificateException {
      this(cryptoProvider);
      Certificate decodedCert = decode(certificateBytes);
      wrap(decodedCert);

      String signingPrivateKeyAlias = "signingPrivateKey_" + getCertID8();
      String encryptionPrivateKeyAlias = "encryptionPrivateKey_" + getCertID8();

      signingPrivateKey = reconstructImplicitPrivateKey(seedPrivateKey, 
         privateKeyReconstructionValueBytes, signingPrivateKeyAlias);
      encryptionPrivateKey = reconstructImplicitPrivateKey(seedPrivateKey, privateKeyReconstructionValueBytes,
         encryptionPrivateKeyAlias);
   }

   private SecureECPrivateKey reconstructImplicitPrivateKey(
      SecureECPrivateKey seedPrivateKey,
      byte[] privateKeyReconstructionValueBytes,
      String signingPrivateKeyAlias) {

      //TODO
      log.error("reconstructImplicitPrivateKey NOT Implemented");
      return null;
   }
    ...
    *           cryptographic provider to use
    * @param certificateBytes
    *           encoded byte array to decode certificate from
    * @return decoded and wrapped certificate
    * @throws CertificateException
    *            on certificate error
    */
    ...
    *            on certificate error
    */
   static public CertificateWrapper fromBytes(
    ...
    * 
    * @return private encryption key or null if the key is not present in the
    *         certificate
    */
   public SecureECPrivateKey getEncryptionPrivateKey() {
    ...
    *           new private encryption key value
    */
   public void setEncryptionPrivateKey(SecureECPrivateKey encryptionPrivateKey) {
    ...
    * 
    * @return public signing key or null if the key is not present in the
    *         certificate
    */
    ...
    * 
    * @return private signing key or null if the key is not present in the
    *         certificate
    */
   public SecureECPrivateKey getSigningPrivateKey() {
    ...
    *           new private signing key value
    */
   public void setSigningPrivateKeyAlias(SecureECPrivateKey signingPrivateKey) {
      this.signingPrivateKey = signingPrivateKey;
   }

   /**
    * Verifies that certificate is valid i.e. start date is valid, not expired,
    * and not revoked
    * 
   ...
   /**
    * Retrieves certificate HashedId8 (a.k.a. digest) which is the low-order 8
    * octets of the hash of that certificate obtained using SHA-256 as specified
    * in the FIPS 180-3
    * 
    ...
    * 
    * @param certificate
    *           the certificate to wrap
    * @param protParam
    * @throws CertificateException
    */
   private void wrap(Certificate certificate)
         throws CertificateException {
      try {
         ...
         if (certType != CertificateType.explicit && certType != CertificateType.implicit) {
            throw new CertificateException(String.format(
               ...
               certType.longValue(), CertificateType.explicit.longValue(), CertificateType.implicit.longValue()));
         }
         ...
         if (certType == CertificateType.explicit) {
            ...
            EccP256CurvePoint verificationKeyPoint = (verificationKey.hasEcdsaNistP256())
                  ? (verificationKey.getEcdsaNistP256()) : (verificationKey.getEcdsaBrainpoolP256r1());
            signingPublicKey = ecdsaProvider.decodePublicKey(verificationKeyPoint);

            /*
             * // TODO: Turn back on verification of signature when we are able to
             * test with valid certs // Decode and verify signature
             * EcdsaP256SignatureWrapper signature =
             * EcdsaP256SignatureWrapper.decode(certificate.getSignature(),
             * ecdsaProvider); byte[] tbsBytes =
             * Ieee1609dot2Helper.encodeCOER(toBeSigned); if
             * (!cryptoHelper.verifySignature(tbsBytes,
             * signerCertificate.getBytes(),
             * signerCertificate.getSigningPublicKey(), signature)) { throw new
             * CertificateException("Certificate signature is not valid"); }
             */
         } else {
         ...
         if (toBeSigned.hasEncryptionKey()) {
            ...
            if (!encryptionKey.getSupportedSymmAlg().equalTo(SymmAlgorithm.aes128Ccm)) {
               throw new CertificateException(
                     "Unexpected public key algorithm value: " + encryptionKey.getSupportedSymmAlg());
            }
         ...
         throw new CertificateException("Error wrapping certificate!", e);
      }
   }
   ...
   private CertificateWrapper getSignerCertificate(HashedId8 signerCertId8) {
      ...
      if (signerCertificate == null) {
         ...
         log.warn(
            "Signer certificate with CertId8 " + id + " not found in the certificate store. Falling back for root CA");
    ...
    * 
    * @param newRootPublicCertificateFriendlyName
    *           new friendly name
    */
    ...
    * 
    * @param cert
    *           certificate to retrieve the unsigned data from
    * @param certBytes
    *           the encoded certificate bytes
